...
with yaspin(text="Executing Script", color="cyan") as sp:
    ...
    verify_path = f"{base_path}/{instance}/Audit"
    os.makedirs(verify_path, exist_ok=True)
    logging.basicConfig(filename=f'{verify_path}/verify-{periodId}.log', filemode='a', level=logging.INFO, format='%(message)s')
    verify_restart_file=f"verify-{periodId}.csv"
    
    file_exists = os.path.isfile(f"{verify_path}/{verify_restart_file}")
    if not file_exists:
        with open(f"{verify_path}/{verify_restart_file}", 'w') as file:
                    file.write('reconciliationId\n')
                    
    utility = DataProcessingUtility(max_workers=10, config_path=config_path,periodPath=periodfile,periodId=periodId)
    restartDf = pd.read_csv(f"{verify_path}/{verify_restart_file}")
    ...
    # print(df)
    try:
        with open(f"{verify_path}/{verify_restart_file}", mode='a', newline='') as file:
            ...
            for i,row in filtered_df.iterrows():
                ...
                try:
                    ...
                    if os.path.exists(filePath):
                        ...
                        logging.info(f'localPath={directory_path.replace(f"{base_path}/","")}')
                        logging.info(f"summaryURL: {base_url}/account/reconciliation/{recId} ")
                        logging.info(f"Summary File: {filename}")
                        if support_files>0:
                            logging.info("support Files:")
                            for file in files:
                                logging.info(f"\t{file}")
